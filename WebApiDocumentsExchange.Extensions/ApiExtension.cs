...
namespace WebApiDocumentsExchange.Extensions
{
    public static class ApiExtension
    {
        ...
        #region Queries
        public static async Task<long> ApiPostQueryAsync(this HttpClient client, Guid destination, string objectTypeName, string objectId)
        {
            //client.DefaultRequestHeaders.Add("apiToken", token);
            var content = new PostObject(destination, objectTypeName, objectId,DateTime.Now,string.Empty);
            var response = await client.PostAsJsonAsync<PostObject>("api/v0/queries", content);
            if (!response.IsSuccessStatusCode)
                throw new Exception($"The error post query:{objectTypeName}");
            var result = await response.Content.ReadAsStringAsync();
            return long.Parse(result);
        }
        public static async Task<long> ApiPostQueryAsync(this HttpClient client, Guid destination, string objectTypeName, Guid objectId) => await ApiPostQueryAsync(client, destination, objectTypeName, objectId.ToString());
